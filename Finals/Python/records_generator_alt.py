...
and upload each table as an object to S3
"""
...
def generate_records():
    """
    First run for populating all tables
    ...
    # create mics table
    mics=pd.DataFrame(generator._mic_records, columns=['mic_id','manufacturer','model','type'])
    ...
    # create source table
    sources=pd.DataFrame(generator._source_records, columns=['source_id','source_name'])
    ...
    venue_records = []
    for i in range(100):
        venue_name=generator.generate_venue_name()
        venue_size=generator.generate_venue_size()
        venue_reverberance_length=generator.generate_reverberance_length(venue_size)
    ...
    for i in range(25):
        # generate the band
        band_records.append([i, generator.generate_band_name(), random.randint(4,10), generator.generate_style()])
        # generate members
        ...
        # bass drum
        generator.add_micused_fact_drums('Kick', mic_used_records, member_records, fact_records, i)
       
        # snare drum
        generator.add_micused_fact_drums('Snare', mic_used_records, member_records, fact_records, i)
        
        # hi-hats
        generator.add_micused_fact_drums('Hi-hat', mic_used_records, member_records, fact_records, i)
        
        # toms
        generator.add_micused_fact_drums('Toms', mic_used_records, member_records, fact_records, i)
        
        # overhead
        generator.add_micused_fact_drums('Overhead', mic_used_records, member_records, fact_records, i)
        ...
        member_records.append([member_records[-1][0]+1, i, generator.generate_role()])
